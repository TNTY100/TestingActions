# Changelog

All notable changes to IsotopeTrack are documented here.


---

## v0.0.17 - 2026-08-13

### What’s Changed

* No changes

## v0.0.15 - 2026-08-13

### What’s Changed

* Dev (#17) @TNTY100

## v0.0.14 - 2026-08-13

### What’s Changed

* Update ToBeCompiled.txt - Dev modifications3 (#16) @TNTY100

## — 2026-08-13

- Dev (Pr: #12 by @TNTY100)
- Update ToBeCompiled.txt (Pr: #11 by @TNTY100)

**Full Changelog**: https://github.com/Houssame-EA/IsotopeTrack/compare/v0.0.10...

## — 2026-08-13

No change via Pull Request. Change log should be done manually.

## — 2026-08-13

- Update ToBeCompiled.txt - Test6 (Pr: #9 by @TNTY100)
- Update ToBeCompiled.txt - Test7 (Pr: #10 by @TNTY100)

**Full Changelog**: https://github.com/Houssame-EA/IsotopeTrack/compare/v0.0.2...

## — 2026-08-13

- Update ToBeCompiled.txt - Test6 (Pr: #9 by @TNTY100)
- Update ToBeCompiled.txt - Test7 (Pr: #10 by @TNTY100)

**Full Changelog**: https://github.com/Houssame-EA/IsotopeTrack/compare/v0.0.2...

## — 2026-08-13

No change via Pull Request. Change log should be done manually.


---

## — 2026-08-13

> > > > > > > main

- Update ToBeCompiled.txt - Test3 (Pr: #4 by @TNTY100)
- Update ToBeCompiled.txt - Test4 (Pr: #5 by @TNTY100)
- Update ToBeCompiled.txt - Test5 (Pr: #7 by @TNTY100)
- V0.0.3 (Pr: #6 by @TNTY100)
- V0.0.3 (Pr: #8 by @TNTY100)

**Full Changelog**: https://github.com/Houssame-EA/IsotopeTrack/compare/v0.0.1...
